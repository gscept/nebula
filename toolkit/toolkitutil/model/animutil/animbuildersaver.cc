...
AnimBuilderSaver::Save(const URI& uri, const Util::Array<AnimBuilder>& animBuilders, Platform::Code platform)
{
    ...
    if (stream->Open())
    {
        ...
        AnimBuilderSaver::WriteClips(stream, animBuilders, byteOrder);
        AnimBuilderSaver::WriteKeys(stream, animBuilders, byteOrder);
...
AnimBuilderSaver::WriteHeader(const Ptr<Stream>& stream, const Util::Array<AnimBuilder>& animBuilders, const ByteOrder& byteOrder)
{
    ...
    nax3Header.numAnimations = byteOrder.Convert<uint>(animBuilders.Size());
...
AnimBuilderSaver::WriteAnimations(const Ptr<IO::Stream>& stream, const Util::Array<AnimBuilder>& animBuilders, const System::ByteOrder& byteOrder)
{
    ...
    for (auto& anim : animBuilders)
    {
        ...
        nax3.numKeys = anim.CountKeys();
        nax3.firstClip = clipOffset;
        nax3.firstKey = keyOffset;

        clipOffset += nax3.numClips;
        keyOffset += nax3.numKeys;
...
*/
void
AnimBuilderSaver::WriteClips(const Ptr<Stream>& stream, const Util::Array<AnimBuilder>& animBuilders, const ByteOrder& byteOrder)
{
    for (auto& anim : animBuilders)
    {
...
*/
void
AnimBuilderSaver::WriteKeys(const Ptr<Stream>& stream, const Util::Array<AnimBuilder>& animBuilders, const ByteOrder& byteOrder)
{
    for (auto& anim : animBuilders)
    {
        SizeT numClips = anim.GetNumClips();
