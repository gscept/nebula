...
Spinlock::operator=(Spinlock&& rhs)
{
    ...
    while (Interlocked::CompareExchange((long*)&this->lock, InvalidThreadId, InvalidThreadId) != InvalidThreadId)
    {
        Thread::YieldThread();
    }
}
...
Spinlock::Lock()
{
    ...
    while (Interlocked::CompareExchange((long*) &this->lock, threadId, InvalidThreadId) != InvalidThreadId)
    {
        Thread::YieldThread();
    }
}
...
Spinlock::Unlock()
{
    ...
    Threading::Interlocked::Exchange((long*) &this->lock, InvalidThreadId);
}
