...
*/
void
BruteforceSystem::Run(const Threading::Interlocked::AtomicCounter* previousSystemCompletionCounters, const Util::FixedArray<const Threading::Interlocked::AtomicCounter*, true>& extraCounters)
{
    IndexT i;
    for (i = 0; i < this->obs.count; i++)
    {
        ...
        // Setup counters
        Util::FixedArray<const Threading::Interlocked::AtomicCounter*, true> counters(extraCounters.Size() + (previousSystemCompletionCounters == nullptr ? 0 : 1));
