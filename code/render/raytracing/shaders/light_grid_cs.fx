...
void csClusterAABB()
{
    ...
    vec3 offsetMin = vec3(index3D.x - NumCells.x * 0.5f, index3D.y - NumCells.y * 0.5f, index3D.z - NumCells.z * 0.5f) * 10.0f;
    vec3 offsetMax = vec3(index3D.x + 1 - NumCells.x * 0.5f, index3D.y + 1 - NumCells.y * 0.5f, index3D.z + 1 - NumCells.z * 0.5f)* 10.0f;

    // Calculate AABB using min and max
    ClusterAABB aabb;
    vec3 eye = EyePos.xyz;
    aabb.minPoint = vec4(eye.xyz + offsetMin * BlockSize.x, 1);
    aabb.maxPoint = vec4(eye.xyz + offsetMax * BlockSize.x, 1);
