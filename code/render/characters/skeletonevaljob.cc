...
SkeletonEvalJob(const Jobs::JobFuncContext& ctx)
{
	...
	for (ptrdiff sliceIdx = 0; sliceIdx < ctx.numSlices; sliceIdx++)
	{
		...
		const float4* samplesBase = (const float4*)(ctx.inputs[1] + sliceIdx * ctx.inputSizes[1]);
		const float4* samplesPtr = samplesBase;

		matrix44* scaledMatrixBase = (matrix44*)(ctx.outputs[0] + sliceIdx * ctx.outputSizes[0]);
		matrix44* skinMatrixBase = (matrix44*)(ctx.outputs[1] + sliceIdx * ctx.outputSizes[1]);
...
SkeletonEvalJobWithVariation(const Jobs::JobFuncContext& ctx)
{
	...
	for (ptrdiff sliceIdx = 0; sliceIdx < ctx.numSlices; sliceIdx++)
	{
		...
		n_assert(0 != compsBase);
		const float4* samplesBase = (const float4*)N_JOB_INPUT(ctx, sliceIdx, 1);
		const float4* samplesPtr = samplesBase;

		matrix44* scaledMatrixBase = (matrix44*)N_JOB_OUTPUT(ctx, sliceIdx, 0);
		matrix44* skinMatrixBase = (matrix44*)N_JOB_OUTPUT(ctx, sliceIdx, 1);
