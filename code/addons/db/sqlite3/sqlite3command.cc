            ...
            switch (this->valueTable->GetColumnValueType(valueTableColumnIndex))
            {
                case Attr::IntType:
                 ...
                 case Attr::Int64Type:
                    {
                        n_assert(SQLITE_INTEGER == sqliteColumnType);
                        int64_t val = sqlite3_column_int64(this->sqliteStatement, resultColumnIndex);                        
                        this->valueTable->SetInt64(valueTableColumnIndex, rowIndex, val);
                    }
                    break;
                 ...
                 case Attr::UIntType:
                    {
                        n_assert(SQLITE_INTEGER == sqliteColumnType);
                        unsigned int val = (unsigned int)sqlite3_column_int(this->sqliteStatement, resultColumnIndex);                        
                        this->valueTable->SetUInt(valueTableColumnIndex, rowIndex, val);
                    }
                    break;
...
*/
void
Sqlite3Command::BindInt64(IndexT index, int64_t val)
{
    n_assert(0 != this->sqliteStatement);
...
//------------------------------------------------------------------------------
/**
    Bind an unsigned integer value by placeholder index. Either get the index from
    one of the IndexOf methods or use a 0-based index.
*/
void
...
*/
void
Sqlite3Command::BindInt64(const Util::String& name, int64_t val)
{
    Sqlite3Command::BindInt64(Sqlite3Command::IndexOf(name), val);
}
...
*/
void
Sqlite3Command::BindUInt(const Util::String& name, unsigned int val)
{
    Sqlite3Command::BindUInt(Sqlite3Command::IndexOf(name), val);
}
...
*/
void
Sqlite3Command::BindInt64(const Attr::AttrId& id, int64_t val)
{
    Sqlite3Command::BindInt64(Sqlite3Command::IndexOf(id), val);
}
