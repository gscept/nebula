...
PYBIND11_EMBEDDED_MODULE(nmath, m)
{
    py::class_<Math::float4>(m, "Float4", py::buffer_protocol())
		...
		.def(py::self /= py::self)
		.def(-py::self)
		.def_property("x", &Math::float4::get_x, &Math::float4::set_x)
		.def_property("y", &Math::float4::get_y, &Math::float4::set_y)
		.def_property("z", &Math::float4::get_z, &Math::float4::set_z)
		.def_property("w", &Math::float4::get_z, &Math::float4::set_w)
