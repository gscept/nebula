...
{
__ImplementClass(Multiplayer::MultiplayerServer, 'MPSV', Core::RefCounted);
...
static MultiplayerServer* callbackServerInstance = nullptr;
static void
...
MultiplayerServer::Open()
{
    ...
    this->pollGroup = this->netInterface->CreatePollGroup();
...
MultiplayerServer::Close()
{
    ...
    this->netInterface->DestroyPollGroup(this->pollGroup);
...
*/
void
MultiplayerServer::AddClientConnection(ClientConnection* conn)
{
    n_assert(conn != nullptr);
    this->clientConnections.Add(conn->connectionId, conn);
}
...
*/
void
MultiplayerServer::OnNetConnectionStatusChanged(SteamNetConnectionStatusChangedCallback_t* info)
{
    char temp[1024];
    switch (info->m_info.m_eState)
    {
        ...
        case k_ESteamNetworkingConnectionState_Connecting:
        {
            ...
            if (!this->netInterface->SetConnectionPollGroup(info->m_hConn, this->pollGroup))
            {
                this->netInterface->CloseConnection(info->m_hConn, 0, nullptr, false);
            ...
            else
            {
                ...
                //    SendStringToClient(info->m_hConn, c.second.m_sNick.c_str());
            }
...
*/
void
MultiplayerServer::PollIncomingMessages()
{
    char temp[1024];

    ISteamNetworkingMessage* incomingMsg = nullptr;
    int numMsgs = this->netInterface->ReceiveMessagesOnPollGroup(this->pollGroup, &incomingMsg, this->maxMessagesPerFrame);
    if (numMsgs == 0)
    {
        return;
    }
    ...
    {
        msg = incomingMsg + i;
        IndexT clientIndex = this->clientConnections.FindIndex(msg->m_conn);
        n_assert(clientIndex != InvalidIndex);
        
        void* data = msg->m_pData;
        int bytes = msg->m_cbSize;

        // TODO: Parse with flatbuffers?
...
*/
void
MultiplayerServer::PollConnectionChanges()
{
...
*/
void
MultiplayerServer::PushPendingMessages()
{
    if (this->clientConnections.IsEmpty())
        return;

    if (this->sendTimer.GetTime() < this->tickRate)
    {
        return;
    }
