...
public:
    template<typename T>
    explicit StateDescription(Util::StringAtom name, T const& defaultValue)
    {
    ...
    StateDescription(StateDescription&& desc) : defVal(desc.defVal), typeSize(desc.typeSize), name(desc.name), fTable(desc.fTable), trivialType(desc.trivialType)
    {
        desc.defVal = nullptr;
        desc.name = nullptr;
        desc.typeSize = 0;
    ...
    void operator=(StateDescription&& rhs) noexcept
    {
