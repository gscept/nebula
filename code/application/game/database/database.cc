...
Database::SetToDefault(TableId tid, IndexT row)
{
	...
	for (IndexT col = 0; col < table.columns.Size(); col++)
	{
		...
		case Game::AttributeType::Vec2Type:			*((Math::vec2*)buffer + row) = std::get<Math::vec2>(column.GetDefaultValue()); break;
		case Game::AttributeType::Vec4Type:			*((Math::vec4*)buffer + row) = std::get<Math::vec4>(column.GetDefaultValue()); break;
		case Game::AttributeType::QuatType:			*((Math::quat*)buffer + row) = std::get<Math::quat>(column.GetDefaultValue()); break;
		case Game::AttributeType::Mat4Type:			*((Math::mat4*)buffer + row) = std::get<Math::mat4>(column.GetDefaultValue()); break;
...
Database::Set(TableId tid, ColumnId col, IndexT row, AttributeValue const& value)
{
	...
	case Game::AttributeType::Vec2Type:			n_assert(std::holds_alternative<Math::vec2>(value)); *((Math::vec2*)buffer + row) = std::get<Math::vec2>(value); break;
	case Game::AttributeType::Vec4Type:			n_assert(std::holds_alternative<Math::vec4>(value)); *((Math::vec4*)buffer + row) = std::get<Math::vec4>(value); break;
	case Game::AttributeType::QuatType:			n_assert(std::holds_alternative<Math::quat>(value)); *((Math::quat*)buffer + row) = std::get<Math::quat>(value); break;
	case Game::AttributeType::Mat4Type:			n_assert(std::holds_alternative<Math::mat4>(value)); *((Math::mat4*)buffer + row) = std::get<Math::mat4>(value); break;
...
Database::EraseSwapIndex(Table& table, InstanceId instance)
{
	...
	for (int i = 0; i < cols.Size(); ++i)
	{
		...
		case Game::AttributeType::Vec2Type:			EraseSwap<Math::vec2>(cols[i], buffers[i], instance.id, end); break;
		case Game::AttributeType::Vec4Type:			EraseSwap<Math::vec4>(cols[i], buffers[i], instance.id, end); break;
		case Game::AttributeType::QuatType:			EraseSwap<Math::quat>(cols[i], buffers[i], instance.id, end); break;
		case Game::AttributeType::Mat4Type:			EraseSwap<Math::mat4>(cols[i], buffers[i], instance.id, end); break;
...
Database::GrowTable(TableId tid)
{
	...
	for (int i = 0; i < colTypes.Size(); ++i)
	{
		...
		case Game::AttributeType::Vec2Type:			GrowBuffer<Math::vec2>(colTypes[i], buffers[i], oldCapacity, table.numRows, table.capacity); break;
		case Game::AttributeType::Vec4Type:			GrowBuffer<Math::vec4>(colTypes[i], buffers[i], oldCapacity, table.numRows, table.capacity); break;
		case Game::AttributeType::QuatType:			GrowBuffer<Math::quat>(colTypes[i], buffers[i], oldCapacity, table.numRows, table.capacity); break;
		case Game::AttributeType::Mat4Type:			GrowBuffer<Math::mat4>(colTypes[i], buffers[i], oldCapacity, table.numRows, table.capacity); break;
...
Database::AllocateColumn(TableId tid, Column column)
{
	...
	case Game::AttributeType::Vec2Type:			buffer = AllocateBuffer<Math::vec2>(column, table.capacity, table.numRows); break;
	case Game::AttributeType::Vec4Type:			buffer = AllocateBuffer<Math::vec4>(column, table.capacity, table.numRows); break;
	case Game::AttributeType::QuatType:			buffer = AllocateBuffer<Math::quat>(column, table.capacity, table.numRows); break;
	case Game::AttributeType::Mat4Type:			buffer = AllocateBuffer<Math::mat4>(column, table.capacity, table.numRows); break;
