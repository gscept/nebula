...
Float4Test::Run()
{
    ...
    v2.z() = 7.0f;
    v2.w() = 8.0f;
    VERIFY(v2 == float4(5.0f, 6.0f, 7.0f, 8.0f));
...
*/
    ...
    VERIFY(n_fequal(v2.length(), 6.244997998f, 0.0001f));
    VERIFY(n_fequal(v2.length3(), 3.741657387f, 0.0001f));
    VERIFY(n_fequal(v2.lengthsq(), 39.0f, 0.0001f));
	...
	// 3 components
	VERIFY(!float4::less3_any(v1222, v1111));
	VERIFY( float4::less3_any(v1222, v2222));
	VERIFY(!float4::less3_all(v1111, v1222));
	VERIFY( float4::less3_all(v0000, v1222));
	VERIFY( float4::lessequal3_any(v1111, v1222));
	VERIFY(!float4::lessequal3_any(v2222, v1111));
	VERIFY(!float4::lessequal3_all(v1222, v1111));
	VERIFY( float4::lessequal3_all(v2222, v2222));
	VERIFY( float4::lessequal3_all(v1222, v2222));
	VERIFY(!float4::greater3_any(v1222, v2222));
	VERIFY( float4::greater3_any(v2222, v1222));
	VERIFY( float4::greater3_all(v1222, v0000));
	VERIFY(!float4::greater3_all(v1111, v1222));
	VERIFY( float4::greaterequal3_any(v1222, v1111));
	VERIFY(!float4::greaterequal3_any(v0000, v1222));
	VERIFY(!float4::greaterequal3_all(v0011, v1222));
	VERIFY( float4::greaterequal3_all(v1222, v1111));
	VERIFY( float4::greaterequal3_all(v0000, v0000));
	VERIFY( float4::greaterequal3_all(v1111, v0000));
	VERIFY(!float4::equal3_any(v1111, v0000));
	VERIFY( float4::equal3_any(v1111, v1222));
	VERIFY( float4::equal3_any(v1111, v1111));
	VERIFY( float4::equal3_all(v1111, v1111));
	VERIFY(!float4::equal3_all(v1111, v1222));    
	VERIFY( float4::nearequal3(v0000, float4( 0.001f, 0.1f, 0.05f, 3.0f), float4(0.001f, 0.1f, 0.05f, 0.0f)));
	VERIFY( float4::nearequal3(v0000, float4(-0.001f,-0.1f,-0.05f,-3.0f), float4(0.001f, 0.1f, 0.05f, 0.0f)));
	VERIFY( !float4::nearequal3(v0000, float4( 0.002f, 0.1f, 0.05f, 3.0f), float4(0.001f, 0.1f, 0.05f, 0.0f)));
	VERIFY( !float4::nearequal3(v0000, float4(-0.001f,-0.2f,-0.05f,-3.0f), float4(0.001f, 0.1f, 0.05f, 0.0f)));
	// 4 components
	VERIFY(!float4::less4_any(v2222, v1222));    
	VERIFY( float4::less4_any(v1111, v1222));    
	VERIFY( float4::less4_any(v1222, v2222));    
	VERIFY(!float4::less4_all(v1111, v1222));
	VERIFY( float4::less4_all(v0000, v1222));
	VERIFY( float4::lessequal4_any(v1111, v1222));
	VERIFY(!float4::lessequal4_any(v1222, v0000));
	VERIFY( float4::lessequal4_all(v1111, v1222));
	VERIFY( float4::lessequal4_all(v1111, v1111));
	VERIFY(!float4::lessequal4_all(v1222, v1111));
	VERIFY(!float4::greater4_any(v1111, v1111));
	VERIFY(!float4::greater4_any(v0000, v1111));
	VERIFY( float4::greater4_any(v1222, v1111));
	VERIFY(!float4::greater4_all(v1222, v1111));
	VERIFY( float4::greater4_all(v1111, v0000));
	VERIFY( float4::greaterequal4_any(v1222, v2222));
	VERIFY(!float4::greaterequal4_any(v0000, v1111));
	VERIFY( float4::greaterequal4_any(v0000, v0000));
	VERIFY( float4::greaterequal4_all(v0000, v0000));
	VERIFY(!float4::greaterequal4_all(v0000, v0011));
	VERIFY( float4::greaterequal4_all(v1111, v0000));
	VERIFY( float4::equal4_any(v0011, v0000));
	VERIFY(!float4::equal4_any(v1222, v0000));
	VERIFY(!float4::equal4_all(v0011, v0000));
	VERIFY(!float4::equal4_all(v1222, v0000));
	VERIFY( float4::equal4_all(v0000, v0000));
	VERIFY( float4::nearequal4(v0000, float4( 0.001f, 0.1f, 0.05f, 3.0f), float4(0.001f, 0.1f, 0.05f, 3.0f)));
	VERIFY( float4::nearequal4(v0000, float4(-0.001f,-0.1f,-0.05f,-3.0f), float4(0.001f, 0.1f, 0.05f, 3.0f)));
	VERIFY( !float4::nearequal4(v0000, float4( 0.002f, 0.1f, 0.05f, 3.0f), float4(0.001f, 0.1f, 0.05f, 3.0f)));
	VERIFY( !float4::nearequal4(v0000, float4(-0.001f,-0.2f,-0.05f,-3.0f), float4(0.001f, 0.1f, 0.05f, 3.0f)));

    // check splat
    v1.set(1.0f, 2.0f, 3.0f, 4.0f);
    v0 = float4::splat(v1, 0);
    VERIFY(v0 == float4(1.0f, 1.0f, 1.0f, 1.0f));
    v0 = float4::splat_x(v1);
    VERIFY(v0 == float4(1.0f, 1.0f, 1.0f, 1.0f));
    v0 = float4::splat(v1, 1);
    VERIFY(v0 == float4(2.0f, 2.0f, 2.0f, 2.0f));
    v0 = float4::splat_y(v1);
    VERIFY(v0 == float4(2.0f, 2.0f, 2.0f, 2.0f));
    v0 = float4::splat(v1, 2);
    VERIFY(v0 == float4(3.0f, 3.0f, 3.0f, 3.0f));
    v0 = float4::splat_z(v1);
    VERIFY(v0 == float4(3.0f, 3.0f, 3.0f, 3.0f));
    v0 = float4::splat(v1, 3);
    VERIFY(v0 == float4(4.0f, 4.0f, 4.0f, 4.0f));
	...
	VERIFY(float4::permute(v0,v1,0,1,5,6) == float4(1.0f,2.0f,6.0f,7.0f));

	// floor/ceil

	v1.set(1.3, 2.5, 3.8, 5.0);
	v0 = float4::ceiling(v1);
	VERIFY(v0 == float4(2.0f, 3.0f, 4.0f, 5.0f));
	v0 = float4::floor(v1);
	VERIFY(v0 == float4(1.0f, 2.0f, 3.0f, 5.0f));
}
